...
from scripts.postprocess import _block_grid_meta, aggregate_blocks
...
def _build_bundle(con: duckdb.DuckDBPyConnection, aoi_code: str) -> bytes:
    ...
    for row in cur.fetchall():
        (bid, geojson, height_m, insar_h, insar_sigma, fused_h,
         ...
         t_slope, s_amp, t_r2, fmode,
         v_sig, v_ew_sig, c_comp_pct, c_shear_pct, c_size,
         blk_id, c_block_pct,
         closure_r, dem_e, dem_f) = row
        bids.append(bid)
        heights.append(height_m if height_m is not None else 0.0)
        insar_heights.append(insar_h if insar_h is not None else 0.0)
        insar_height_sigmas.append(insar_sigma if insar_sigma is not None else 0.0)
    ...
    # building_id and bids is sorted by building_id too, reshape is direct.
    assert len(arr["building_id"]) == n_buildings * n_months, (
        f"expected {n_buildings * n_months} rows, got {len(arr['building_id'])}"
    )
...
@app.get("/buildings/at-date", response_model=list[BuildingAtDate])
